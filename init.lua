...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader><enter>', builtin.oldfiles, { desc = '[<enter>] Search resent files' })
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
      --  This function gets run when an LSP attaches to a particular buffer. (every time a new file is opened that is associated with an lsp)
      vim.api.nvim_create_autocmd('LspAttach', {
        group = vim.api.nvim_create_augroup('kickstart-lsp-attach', { clear = true }),
        callback = function(event)
          local map = function(keys, func, desc, mode) -- Helper function for setting keybindings
            mode = mode or 'n'
